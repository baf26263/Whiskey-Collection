...
def create_whiskey(current_user_token):
    ...
    bottle_size = request.json['bottle_size']
    year_made = request.json['year_made']
    user_token = current_user_token.token

    print(f'BIG TESTER: {current_user_token.token}')

    whiskey = Whiskey(name, origin, bottle_size, year_made, user_token = user_token )
...
def get_single_whiskey(current_user_token, id):
    book = Whiskey.query.get(id)
    response = whiskey_schema.dump(book)
    return jsonify(response)
...
def update_whiskey(current_user_token,id):
    ...
    whiskey.bottle_size = request.json['bottle_size']
