...
class Whiskey(db.Model):
    ...
    bottle_size = db.Column(db.String(100))
    year_made = db.Column(db.String(4))
    user_token = db.Column(db.String, db.ForeignKey('user.token'), nullable = False)

    def __init__(self,name,origin,bottle_size,year_made,user_token, id = ''):
        self.id = self.set_id()
        self.name = name
        self.origin = origin
...
class WhiskeySchema(ma.Schema):
    class Meta:
        fields = ['id', 'name', 'origin','bottle_size','year_made']
